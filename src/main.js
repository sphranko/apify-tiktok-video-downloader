 ...
 *  - order         {string}   'desc' (newest first) | 'asc' (oldest first). Default: 'desc'.
...
await Actor.main(async () => {
    ...
    if (!Array.isArray(profileUrls) || profileUrls.length === 0) {
        throw new Error(
            '"profileUrls" must be a non-empty array of TikTok profile URLs.',
        );
    }
            ...
            if (downloadVideos && video.video.downloadUrl) {
                ...
                try {
                    ...
                    log.error(
                        ...
                        + 'The dataset record will be saved without a storageUrl.',
                    );
                }
            } else if (downloadVideos && !video.video.downloadUrl) {
                log.warning(`[${video.id}] No download URL available — skipping download.`);
            }
